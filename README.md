...
```
target
├── preamble                the nodes its factory returns
└── div                     group-list
    └── section             group
        ├── span            group heading
        └── ul              entry-list
...
A preamble is the node sequence its factory returns, rendered whenever its target is replaced, including when the index holds no group; without one, the group list is the target's only child. A heading is a `span` by default; a heading factory configured for the target creates the nodes instead, from the tier (`group`, `entry`, or `subentry`), the properties the nodes must carry, and the heading contents. The lists under a heading are always present and always in this order; an empty one holds no items. A locator item holds one link, or a start link, an empty `<span>`, and an end link for a range; a template wraps those nodes without reordering them. A locator link points at the built document, so the source extension becomes `.html` unless it already is `.html`, `.htm`, or `.xhtml`. A cross-reference item holds one link whose contents are the target heading, or the target entry heading, an empty `<span>`, and the target subentry heading when the cross-reference points at a subentry.
